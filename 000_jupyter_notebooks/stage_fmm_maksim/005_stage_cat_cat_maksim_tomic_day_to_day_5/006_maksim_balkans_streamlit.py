...
# lancer le programme
streamlit run 006_maksim_balkans_streamlit.py
...
countries_info = {
    ...
    "Bosnie-Herzégovine": {"capitale": "Sarajevo", "population": "3,280,819", "drapeau": "flags/bosnia_herzegovina.jpg"}
}
...
if selected_country in countries_info:
    ...
    flag_path = country_info['drapeau']

    # Afficher le drapeau
    try:
        flag_image = Image.open(flag_path)
        # Redimensionner l'image à 300x300
        flag_image = flag_image.resize((300, 300))
        st.image(flag_image, caption=f"Drapeau de {selected_country}", use_column_width=False)
